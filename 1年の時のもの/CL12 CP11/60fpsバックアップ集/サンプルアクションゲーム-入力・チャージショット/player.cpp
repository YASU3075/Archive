...
void UpdatePlayer(void)
{
	...
	{//Bを押していないとき
		...
		{//Release処理
			...
			if (charge >= 60)
			{//チャージショット
				SetBullet(player.PosX + 1.0f, player.PosY, 0.5f, 0.0f);					//前
				SetBullet(player.PosX, player.PosY - 1.0f, 0.0f, -0.25f);				//上
				SetBullet(player.PosX - 1.0f, player.PosY, -0.5f, 0.0f);				//左
				SetBullet(player.PosX - 1.0f, player.PosY -1.0f, -0.5f, -0.25f);		//左上
				SetBullet(player.PosX + 1.0f, player.PosY - 1.0f, 0.5f, -0.25f);		//右上
			}
			player.isRelease = true;
			charge = 0;
		}
