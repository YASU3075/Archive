...
void UpdateBullet(void)
{
	for (int i = 0; i < MAX_BULLET; i++)
	{
		...
		{//使用していたら
			...
			if (bullet[i].HitWallCnt > MAX_HIT_COUNT)
			{//跳ね返った回数が最大値を超えたら
			...
			if (bullet[i].PosX <= MIN_BULLET_X || bullet[i].PosX >= MAX_BULLET_X)
			{//左右壁
				...
				//1フレーム前の位置に戻す
				bullet[i].PosX = bullet[i].OldPosX;

			}

			if (bullet[i].PosY <= MIN_BULLET_Y || bullet[i].PosY >= MAX_BULLET_Y)
			{//上下壁
				...
				//1フレーム前の位置に戻す
				bullet[i].PosY = bullet[i].OldPosY;
	...
	for (int i = 0; i < MAX_BULLET; i++)
	{
		...
		//もし描画位置が動いていたら残像処理
			if ((int)bullet[i].OldPosX != (int)bullet[i].PosX || (int)bullet[i].OldPosY != (int)bullet[i].PosY)
			{
				...
				if (
					((int)bullet[i].OldPosX >= MIN_FIELD_X && (int)bullet[i].OldPosX <= MAX_FIELD_X) &&
					((int)bullet[i].OldPosY == FIELD_POSY)
					)
				{//フィールドと重なった時
					//残像の描画位置設定
