...
void UpdateEnemy(void)
{
	...
	{//敵のカウントが増加したとき
		...
		case 26:
			//一周、弾早い
			SetEnemy(10, 2, 1.0f, 0.5f, 2, 25, 2, ENEMYTYPE_TURN, ENEMYBULLETTYPE_FAST);
			break;

		case 27:
			//一周、弾早い
			SetEnemy(35, 3, 0.0f, 0.0f, 9, 20, 1, ENEMYTYPE_NORMAL, ENEMYBULLETTYPE_NORMAL);
			break;
		...
		{//使用していたら
			...
			{//ダメージを食らっていなかったら
				...
				if (enemy[i].AttackFrameCnt >= 60 * enemy[i].AttackInterval)
				{
					...
					if (EnemyCnt == 10 || EnemyCnt == 26 || EnemyCnt == 27)
					{//弾を３方向に発射
						BulletVelX = (pPlayer->PosX + 10.0f - (enemy[i].PosX + BulletPosX)) / 60 * ENEMYBULLET_SPEED;
						SetEnemyBullet(enemy[i].PosX + BulletPosX, enemy[i].PosY + BulletPosY, BulletVelX, BulletVelY, enemy[i].ColorNum / 16, enemy[i].bType);
						BulletVelX = (pPlayer->PosX - 10.0f - (enemy[i].PosX + BulletPosX)) / 60 * ENEMYBULLET_SPEED;
						SetEnemyBullet(enemy[i].PosX + BulletPosX, enemy[i].PosY + BulletPosY, BulletVelX, BulletVelY, enemy[i].ColorNum / 16, enemy[i].bType);
					}
