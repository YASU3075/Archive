...
void InitializeLoad() 
{
	...
	{
	case 0:
		printf("スコヴィル値は唐辛子の辛さを計る単位");
		break;

	case 1:
		printf("跳ね返した弾を取るといいことがあるかも？");
		break;
	...
	case 3:
		printf("世界一辛い唐辛子の名前はキャロライナリーパー");
		break;

	case 4:
		printf("危なくなったらバットを振りまくれ！");
		break;
...
void DrawLoad()
{
	//乱数で16進数の色の数値を決定(白黒以外)
	int ColorNum = (rand() % 14 + 1) * 16 + 15;
	...
	for (int i = 0; i < MAX_CHIRI; i++)
	{
		...
		{//唐辛子の実を描画するとき
			for (int j = 0; j < MAX_LOAD_Y; j++)
			{
				...
				{
					if (ColorNum == (int)0x2F)
					{//唐辛子の葉の部分と同じ緑色の時は明るい緑にする
						ColorNum = (int)0xAF;
					}

					if (ColorNum == (int)0xFF)
					{//唐辛子の実が初期の白と同じとき赤にする
						ColorNum = (int)0x4F;
					}
