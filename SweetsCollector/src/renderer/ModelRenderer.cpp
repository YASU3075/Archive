...
void ModelRenderer::BindGeometry() const
{
	// 頂点バッファ設定
	UINT stride = sizeof(VERTEX_3D);
	UINT offset = 0;
...
void ModelRenderer::DrawGeometyrOnly() const
{
	BindGeometry();
	...
	// サブセット設定
	{
		...
		for( unsigned int i = 0; i < modelObj.SubsetNum; i++ )
		{
			...
			if (Model->SubsetArray[i].Material.Texture)
				Model->SubsetArray[i].Material.Material.m_texture_enable = true;
			else
				Model->SubsetArray[i].Material.Material.m_texture_enable = false;
...
void ModelRenderer::LoadMaterial( const char *FileName, MODEL_MATERIAL **MaterialArray, unsigned int *MaterialNum )
{
	...
	while( true )
	{
		...
		if( strcmp( str, "newmtl" ) == 0 )
		{
			...
			materialArray[mc].Material.m_emission.x = 0.0f;
			materialArray[mc].Material.m_emission.y = 0.0f;
			materialArray[mc].Material.m_emission.z = 0.0f;
			materialArray[mc].Material.m_emission.w = 0.0f;
		}
		else if( strcmp( str, "Ka" ) == 0 )
		{
			//アンビエント
			fscanf( file, "%f", &materialArray[ mc ].Material.m_ambient.x );
			fscanf( file, "%f", &materialArray[ mc ].Material.m_ambient.y );
			fscanf( file, "%f", &materialArray[ mc ].Material.m_ambient.z );
			materialArray[ mc ].Material.m_ambient.w = 1.0f;
		}
		else if( strcmp( str, "Kd" ) == 0 )
		{
			//ディフューズ
			fscanf( file, "%f", &materialArray[ mc ].Material.m_diffuse.x );
			fscanf( file, "%f", &materialArray[ mc ].Material.m_diffuse.y );
			fscanf( file, "%f", &materialArray[ mc ].Material.m_diffuse.z );
			materialArray[ mc ].Material.m_diffuse.w = 1.0f;
		}
		else if( strcmp( str, "Ks" ) == 0 )
		{
			//スペキュラ
			fscanf( file, "%f", &materialArray[ mc ].Material.m_specular.x );
			fscanf( file, "%f", &materialArray[ mc ].Material.m_specular.y );
			fscanf( file, "%f", &materialArray[ mc ].Material.m_specular.z );
			materialArray[ mc ].Material.m_specular.w = 1.0f;
		}
		else if( strcmp( str, "Ns" ) == 0 )
		{
			//スペキュラ強度
			fscanf( file, "%f", &materialArray[ mc ].Material.m_shininess );
		}
		else if( strcmp( str, "d" ) == 0 )
		{
			//アルファ
			fscanf( file, "%f", &materialArray[ mc ].Material.m_diffuse.w );
		}
